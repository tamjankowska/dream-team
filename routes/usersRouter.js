...
router.post("/signup", (req, res) => {
  ...
  new User({ username, emailAddress, password, reviewer }).save(
    (err, result) => {
...
router.get("/id/:id", (req, res) => {
  User.findOne({ _id: req.params.id }, (err, user) => {
    if (err) {
      ...
      res
        .status(404)
        .json({ err: `The user ${req.params.id} does not exist!` });
    } else {
  ...
  User.findOneAndDelete({ _id: req.params.id }, (err, user) => {
    if (err) {
      ...
      res
        .status(404)
        .json({
          err: `The user ${req.params.id} you are trying to delete does not exist!`,
        });
    } else {
  ...
  User.find({ [req.params.field]: req.params.value }, (err, user) => {
    if ([req.params.field] == "password") {
      res
        .status(403)
        .json({
          msg: `It is not possible to find a user via their password, please use alternative search criteria.`,
        });
    } else if (err) {
      console.log(err);
